    ...
    <div>
        <el-card shadow="never">
            <el-row class="app-btn-group">
                <el-col :span="4">
                    <el-button v-if="$root.CheckPriv($root.Priv.SERVER_PHP_NEW)" @click="openAddDialogHandler" type="primary" size="medium" icon="iconfont left small icon-add">{{ $t('add_php') }}</el-button>&nbsp;
                </el-col>
            ...
            <el-table
                ...
                <el-table-column :label="$t('operate')" width="180" align="right">
                    <template slot-scope="scope">
                        <el-button
                        v-if="$root.CheckPriv($root.Priv.SERVER_PHP_EDIT)"
                        icon="el-icon-edit"
                        type="text"
                        @click="openEditDialogHandler(scope.row)">{{ $t('edit') }}</el-button>
                        <el-button
                        v-if="$root.CheckPriv($root.Priv.SERVER_PHP_DEL)"
                        type="text"
                        icon="el-icon-delete"
                        class="app-danger"
...
<script>
import { listAPPPhpApi, newAPPPhpApi, updateAPPPhpApi, deleteAPPPhpApi, detailAPPPhpApi } from '@/api/php'
export default {
    data() {
        return {
            searchInput: '',
    ...
    methods: {
        ...
        openAddDialogHandler() {
            ...
            this.dialogTitle = this.$t('add_php')
        },
        openEditDialogHandler(row) {
            this.dialogVisible = true
            this.dialogTitle = this.$t('edit_php')
            this.dialogLoading = true
            detailAPPPhpApi({id: row.id}).then(res => {
        ...
        deleteHandler(row) {
            this.$root.ConfirmDelete(() => {
                deleteAPPPhpApi({id: row.id}).then(res => {
        ...
        loadTableData() {
            ...
            listAPPPhpApi({keyword: this.searchInput, offset: this.$root.PageOffset(), limit: this.$root.PageSize}).then(res => {
