<template>
    <el-dialog :width="$root.DialogLargeWidth" :title="dialogTitle" :visible.sync="dialogVisible" @close="dialogCloseHandler">
        <div class="app-dialog" v-loading="dialogLoading">
            <el-form class="app-form" ref="dialogRef" :model="dialogForm" size="medium" label-width="120px" v-if="dialogName=='server'">
                <el-form-item 
                    :label="$t('server_name')"
                    prop="name"
                    :rules="[
...
<script>
import { 
    listSshkeyApi, newServerSshkeyApi, updateServerSshkeyApi, deleteServerSshkeyApi, detailServerSshkeyApi,
    newServerApi, updateServerApi, deleteServerApi, detailServerApi 
} from '@/api/server'
