<template>
  <div>
    <div id="xtermDialog" ref="terminal" style="height:40rem;"></div>
  </div>
...
export default {
  ...
  methods: {
    openXtermDialogHandler(params) {
      console.log(this.server.name, params.server.name)
      if (this.server.name != params.server.name) {
        return;
      }
      serverSession({
  ...
  mounted() {
    ...
    this.$root.BindEventGlobal("closeXtermDialogHandler", this.dialogCloseHandler);
  },
  beforeDestroy() {
    this.$root.UnBindEventGlobal("openXtermDialogHandler");
    this.$root.UnBindEventGlobal("closeXtermDialogHandler");
  }
};
</script>
