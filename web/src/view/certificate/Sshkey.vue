            ...
            <el-table
                ...
                <el-table-column :label="$t('operate')" width="180" align="right">
                    <template slot-scope="scope">
                        <el-button
                        v-if="$root.CheckPriv($root.Priv.SSHKEY_EDIT)"
                        icon="el-icon-edit"
                        type="text"
                        @click="openEditDialogHandler(scope.row)">{{ $t('edit') }}</el-button>
                        <el-button
                        v-if="$root.CheckPriv($root.Priv.SSHKEY_DEL)"
                        type="text"
                        icon="el-icon-delete"
                        class="app-danger"
        ...
        <el-dialog :width="$root.DialogSmallWidth" :title="dialogTitle" :visible.sync="dialogVisible" @close="dialogCloseHandler">
            <div class="app-dialog" v-loading="dialogLoading">
                <el-form ref="dialogRef" :model="dialogForm" size="medium" label-width="80px">
                    ...
                    <el-form-item :label="$t('private_key')">
                        <el-input type="textarea" v-model="dialogForm.private_key"></el-input>
                    </el-form-item>
                </el-form>
...
<script>
import { listSshkeyApi, newSshkeyApi, updateSshkeyApi, deleteSshkeyApi, detailSshkeyApi } from '@/api/server'
export default {
    data() {
        return {
            searchInput: '',
    ...
    methods: {
        ...
        deleteHandler(row) {
            this.$root.ConfirmDelete(() => {
                deleteSshkeyApi({id: row.id}).then(res => {
