...
func Create(model interface{}) bool {
	...
	if err := db.Error; err != nil {
		zoom.App.Logger.Warning("mysql execute error: %s, sql [%v]", err.Error(), db.QueryExpr())
		return false
	}
	return true
}

func CreateWithOmit(model interface{}, columns ...string) bool {
	db := zoom.App.DB.DbHandler.Omit(columns...).Create(model)
	if err := db.Error; err != nil {
		zoom.App.Logger.Warning("mysql execute error: %s, sql [%v]", err.Error(), db.QueryExpr())
		return false
	}
...
func GetMulti(model interface{}, query QueryParam) bool {
	db := zoom.App.DB.DbHandler.Offset(query.Offset)
	if query.Limit > 0 {
	...
	db.Find(model)
	if err := db.Error; err != nil {
		zoom.App.Logger.Warning("mysql query error: %s, sql[%v]", err.Error(), db.QueryExpr())
		return false
	}
	return true
}
...
func Count(model interface{}, count *int, query QueryParam) bool {
	db := zoom.App.DB.DbHandler.Model(model)
	db = parseWhereParam(db, query.Where)
	db = db.Count(count)
	if err := db.Error; err != nil {
		zoom.App.Logger.Warning("mysql query error: %s, sql[%v]", err.Error(), db.QueryExpr())
...
func Delete(model interface{}, query QueryParam) bool {
	if len(query.Where) == 0 {
		zoom.App.Logger.Warning("mysql query error: delete failed, where conditions cannot be empty")
		return false
	}
	db := zoom.App.DB.DbHandler.Model(model)
	db = parseWhereParam(db, query.Where)
